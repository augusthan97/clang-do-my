...
#define TITLE "C 표준 라이브러리와 표준 출력함수(printf)"
...
void printBasic() {
    ...
    //format : 실수형 -> %는 기본적으로 소수섬 이하 6자리 까지만 출력
    //format : 실수형 %.10f : 소숫점 이하 10자리까지 출력

    //printf("char(%c), short(%d), int(%d), float(%f), double(%f)\n", ch, st, integer, real, real2);
    printf("char(%c), short(%d), int(%d), float(%f), double(%.10f) \n", ch, st, integer, real, real2);

    printf("char(%c), short(%d), int(%d), float(%f), double(%.10f) \n", 'b', 4321, 987654321, 0.654321, 0.987654321, 0.987654321);
}
...
void printUnsignedchar() {
    ...
    //printf("char(%c), uchar(%d) (%u) \n", MAX_CHAR, MAX_UCHAR, MAX_UCHAR);
    //printf("char(%d), uchar(%d) (%u) \n", MAX_CHAR, MAX_UCHAR, MAX_UCHAR);
}
